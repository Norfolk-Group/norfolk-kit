...
- **R20. Cowork projects migrate to the new world, each priced to its work.** Inventory Ricardo's Claude Cowork projects and move each into a kit-pattern repo/environment. As part of the move, each project gets a **default driver assignment by cost/quality**: bulk, mechanical, or high-volume work routes to a cheap fleet model (Grok, GLM, Kimi — per R15's launchers); judgment-heavy, investor-facing, or quality-critical work stays on Claude. The existing global cost-discipline rule (cheapest model that does the unit correctly) applies per-project, recorded in each repo's AGENTS.md so the assignment is policy, not per-session memory. Ties into R17's consolidation — Cowork is another surface converging on the one toolchain.
