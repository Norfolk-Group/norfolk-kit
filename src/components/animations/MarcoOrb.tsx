...
export function MarcoOrb({ phase, size = "md", className }: MarcoOrbProps) {
  ...
  return (
    <motion.svg
      ...
      aria-hidden
      animate={{ rotate: 360 }}
      transition={{ duration: rotDur, /* Animation exception: brand/active-build indicator — see nai-design-system Animation Exceptions */ repeat: isTransient ? 0 : Infinity, ease: "linear" }}
    >
      {/*
      ...
      */}
      {Array.from({ length: TRIANGLE_COUNT }).map((_, i) => (
        <motion.polygon
          ...
          transition={{ duration: TRI_PULSE_DUR_S, /* Animation exception: brand/active-build indicator — see nai-design-system Animation Exceptions */ repeat: isTransient ? 0 : Infinity, ease: "easeInOut", delay: i * VERTEX_STAGGER_S }}
        />
      ))}
      <motion.circle
        ...
        transition={{ duration: 1.8, /* Animation exception: brand/active-build indicator — see nai-design-system Animation Exceptions */ repeat: isTransient ? 0 : Infinity, ease: "easeInOut" }}
      />
    </motion.svg>
  );
}
